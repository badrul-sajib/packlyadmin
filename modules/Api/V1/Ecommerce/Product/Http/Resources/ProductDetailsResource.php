...
class ProductDetailsResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'warranty_type' => $this->formatWarrantyType($warranty_type), // $this->warranty_type ? json_decode($this->warranty_type) : null,
            'quantity' => intval($this->total_stock_qty),
            'rating_avg' => $this->rating_avg,
            'rating_count' => $this->rating_count,
            'weight' => $this->weight ?? null,
            'regular_price' => $e_price,
            'discount_price' => $e_discount_price,
    ...
    public function formatWarrantyType($data = [])
    {
        $dummyArr = [];
        ...
        if (is_array($data)) {
            foreach ($data as $key => $value) {
                $dummyArr[$key] = [
                    'status' => intval($value['status']) ?? null,
                    'recurring_type' => intval($value['recurring_type']) ?? null,
                    'recurring_period' => intval($value['recurring_period']) ?? null,
                ];
            }
        }

        return count($dummyArr) > 0 ? $dummyArr : null;
    }
