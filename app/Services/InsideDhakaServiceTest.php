...
class InsideDhakaServiceTest
{
    ...
    private $skipOutsideTermPatterns = [
    ...
    private function checkPlace($input)
    {
    ...
    private function checkSuburb($input)
    {
    ...
    private function checkThana($input)
    {
    ...
    private function getSubstrings($address)
    {
    ...
    public function isInsideDhaka($address)
    {
        $inside_dhaka = false;
        ...
        echo "Normalized address: $address\n";
        echo 'Words: '.implode(', ', $wordsArray)."\n";
        echo 'Substrings: '.implode(', ', $substrings)."\n";
        ...
        foreach ($wordsArray as $word) {
            ...
            if ($matchedOutsideTerm) {
                echo "Outside term found: $word\n";
                echo "Outside term matched: $matchedOutsideTerm\n";
        ...
        foreach ($substrings as $substring) {
            if ($this->checkPlace($substring)) {
                echo 'Place found: '.$this->place['name']."\n";
                $inside_dhaka = true;
            }
            if ($this->checkSuburb($substring)) {
                echo 'Suburb found: '.$this->suburb['name']."\n";
                $inside_dhaka = true;
            }
            if ($this->checkThana($substring)) {
                echo 'Thana found: '.$this->thana['name']."\n";
                $inside_dhaka = true;
            }
        }
        ...
        foreach ($wordsArray as $word) {
            ...
            if ($matchedOutsideTerm) {
                echo "Outside term found: $word\n";
                echo "Outside term word that matches: $matchedOutsideTerm\n";
        ...
        if ($this->suburb && ! $this->thana && ! empty($this->suburb['thana_slug'])) {
            foreach ($this->thanas as $thana) {
                if ($thana['slug'] === $this->suburb['thana_slug']) {
                    $this->thana = $thana;
                    echo 'Thana inferred from suburb: '.$thana['name']."\n";
        ...
        foreach ($wordsArray as $word) {
            if (in_array($word, $this->insideDhakaPostCodes)) {
                echo "Postal code found: $word\n";
                $inside_dhaka = true;
            }
        }

        echo 'Final inside_dhaka: '.($inside_dhaka ? 'true' : 'false')."\n";
